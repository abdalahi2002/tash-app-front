...
export default function Profile() {
  ...
  useEffect(() => {
    const loadProjet = async () => {
      try {
        await checkToken();
        const data = await fetchUser(user.email,authTokens);
        setProfile(data);
      } catch (error) {
        setError(error);
      } finally {
          ...
          <div class="flex flex-col justify-center max-w-xs p-6 shadow-md rounded-xl sm:px-12 dark:bg-gray-50 dark:text-gray-800">
            ...
            <div class="space-y-4 text-center divide-y dark:divide-gray-300">
              <div class="my-2 space-y-1">
                <h2 class="text-xl font-semibold sm:text-2xl">{profile.nom}</h2>
              ...
              <div class="flex justify-center pt-2 space-x-4 align-center">
                <a
                  ...
                  href="#"
                  aria-label="GitHub"
                  class="p-2 rounded-md dark:text-gray-800 hover:dark:text-violet-600"
                >
                  <img
                    src="https://source.unsplash.com/150x150/?portrait?3"
                    alt=""
                    class="w-4 h-4 mx-auto rounded-full dark:bg-gray-500 aspect-square"
                  />
                  {/* <svg
                    viewBox="0 0 496 512"
