...
import { fetchTasks } from "../api/Tache.js";
...
export default function ListeTask() {
  ...
  useEffect(() => {
    const loadProjets = async () => {
      try {
        await checkToken();
        const data = await fetchTasks(authTokens);
        setTasks(data);
      } catch (error) {
        setError(error);
      } finally {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      {error ? (
        ...
        <p className="text-3xl text-gray-600">Aucun Taches Pour vous.</p>
      ) : (
        <div className="min-w-screen min-h-screen flex flex-col items-center justify-center px-5 py-5 space-y-4">
          <div className="flex flex-row justify-between items-center w-full max-w-3xl">
            <h4 className="text-gray-700 text-xl font-semibold">Taches</h4>
